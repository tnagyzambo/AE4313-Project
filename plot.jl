...
function plot_data(name, path)
    ...
    #p3 = plot(df.t, df.u1, label = L"u_1", ylims = (0, 7), xlabel = L"Time (s)", ylabel = L"u (Nm)", legend = :topright)
    ...
    p1 = plot(df.t, df.q1_lvlh, label = L"q_1", ylabel = L"|q|", legend = :topright, xaxis = false, bottom_margin = -5mm, dpi=800)
    p1 = plot!(df.t, df.q2_lvlh, label = L"q_2")
    p1 = plot!(df.t, df.q3_lvlh, label = L"q_3")
    p1 = plot!(df.t, df.q0_lvlh, label = L"q_0")

    #p2 = plot(df.t, df.w1, label = L"\omega_1", ylabel = L"\omega (rad/s)", legend = :topright, xaxis = false, bottom_margin = -5mm)
    p2 = plot(df.t, df.w1_lvlh, label = L"\omega_1", ylabel = L"\omega (rad/s)", xlabel = L"Time (s)", legend = :topright)
    p2 = plot!(df.t, df.w2_lvlh, label = L"\omega_2")
    p2 = plot!(df.t, df.w3_lvlh, label = L"\omega_3")
