...
function plot_data(name, path)
    ...
    p2 = plot(df.t, df.w1_b_lvlh_meas, label = L"\tilde{\omega}_1", color = 1, linestyle = :dot, ylabel = L"\omega (rad/s)", legend = :topright, xaxis = false, bottom_margin = -5mm)
    # p2 = plot(df.t, df.w1_b_lvlh, label = L"\omega_1", ylabel = L"\omega (rad/s)", xlabel = L"Time (s)", legend = :topright)
    p2 = plot!(df.t, df.w2_b_lvlh_meas, label = L"\tilde{\omega}_2", color = 2, linestyle = :dot)
    p2 = plot!(df.t, df.w3_b_lvlh_meas, label = L"\tilde{\omega}_3", color = 3, linestyle = :dot)
    ...
    p2 = plot!(df.t, df.w1_mekf, label = L"\hat{\omega}_1", linestyle = :dash, color = 1)
    p2 = plot!(df.t, df.w2_mekf, label = L"\hat{\omega}_2", linestyle = :dash, color = 2)
    p2 = plot!(df.t, df.w3_mekf, label = L"\hat{\omega}_3", linestyle = :dash, color = 3)
